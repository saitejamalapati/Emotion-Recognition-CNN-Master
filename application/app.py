...
class Pulse(object):
	...
	def loop(self):
		...
		if frame is None:
			print("Inside exit phase")
			print(self.var_emotions)
			self.streamLoop = False
			return
		...
		self.set_emotions_labels(self.e.get_last_prediction())
	
	def set_emotions_labels(self, emotions):
		for i in range(7):
			# f = font.Font(label_pulse, label_pulse.cget("font"))
			# labels_emotion[i].configure(fg='black')
			# labels_emotions_value[i].configure(fg='black')
			e = emotions[i]
			e = "{:0.2f}%".format(e * 100)
			self.var_emotions[i] = (e)
...
# emotions_l = ['Anger', 'disgust', 'Fear', 'Joy', 'Sadness', 'Surprise', 'Indifferent']
